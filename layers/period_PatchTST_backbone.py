...
class TSTiEncoder(nn.Module):  # i means channel-independent
    ...
    def forward(self, x) -> Tensor:  # x: [bs x nvars x patch_len x patch_num]
        ...
        u = self.dropout(u + self.W_pos)  # u: [bs * nvars x patch_num x patch_len]

        # Encoder
        z = self.encoder(u)  # z: [bs * nvars x patch_num x patch_len]
        z = torch.reshape(z, (-1, n_vars, z.shape[-2], z.shape[-1]))  # z: [bs x nvars x patch_num x patch_len]
        z = z.permute(0, 1, 3, 2)  # z: [bs x nvars x patch_len x patch_num]
