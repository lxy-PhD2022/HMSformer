...
from layers.timefre2layer import PatchTST_backbone
...
class Model(nn.Module):
    def __init__(self, configs, max_seq_len: Optional[int] = 1024, d_k: Optional[int] = None, d_v: Optional[int] = None,
        ...
        self.linear = nn.Linear(seq_len, target_window)

        # model
        self.decomposition = decomposition
        if 1:
            self.decomp_module = series_decomp(kernel_size)
            self.model_trend = PatchTST_backbone(seq_len=seq_len, c_in=c_in, context_window=context_window, target_window=target_window,
            ...
            self.model_res = PatchTST_backbone(seq_len=seq_len, c_in=c_in, context_window=context_window, target_window=target_window,
                                               patch_len=patch_len, stride=stride,
                                               max_seq_len=max_seq_len, n_layers=n_layers, d_model=d_model,
                                               n_heads=n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, norm=norm,
                                               attn_dropout=attn_dropout,
    ...
    def forward(self, x):  # x: [Batch, Input length, Channel]
        ...
        # print(res_init.shape)
        res = self.Fourier(res_init) + res_init
        # print(res.shape)
        x = res + trend_init
